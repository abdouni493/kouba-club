...
export interface EmailAttachment {
  name: string;
  /** raw base64 content, without the "data:...;base64," prefix */
  content: string;
}
...
export async function sendClubEmail(
  club: ClubInfo, to: EmailRecipient[], subject: string, htmlContent: string, attachments?: EmailAttachment[],
): Promise<void> {
  ...
  const body: Record<string, unknown> = { sender: { name: club.name || 'Club', email: senderEmail }, to: recipients, subject, htmlContent };
  ...
  if (!res.ok) {
    ...
    throw new Error(body2?.message || `Échec de l'envoi (${res.status})`);
  }
}
